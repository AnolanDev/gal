      ...
      <div class="flex justify-between items-center">
        <div>
          ...
          <p class="text-sm text-gray-600 mt-1">
            Actualiza la información del estudiante {{ estudiante?.nombre_completo || 'Cargando...' }}
          </p>
        </div>
          ...
          <form @submit.prevent="submitForm" class="p-8">
            ...
            <div class="flex justify-between pt-6 border-t">
              ...
              <div class="flex space-x-3">
                ...
                <!-- Botón Guardar siempre disponible si hay datos mínimos -->
                <button
                  type="submit"
                ...
                >
                  ...
                  <span v-else>
                    ...
                    {{ esUltimoPaso ? 'Actualizar Estudiante' : 'Guardar Cambios' }}
                  </span>
                </button>
              </div>
            </div>
...
// Extraer datos del wrapper Resource si es necesario
...
} = useFormularioEstudiante(datosEstudiante.value)

// Watch para actualizar el formulario cuando cambien los props
watch(() => datosEstudiante.value, (nuevosData) => {
  if (nuevosData && nuevosData.id) {
    actualizarFormulario(nuevosData)
  }
...
const submitForm = () => {
  if (!datosEstudiante.value?.id) {
    return
  }

  // Crear copia de datos y filtrar campos problemáticos
  const formData = { ...form.data() }
  
  // Eliminar foto si NO es un File nuevo (null, string de archivo existente, etc.)
  ...
  // Usar método transform de Inertia para filtrar datos
  form.transform(data => formData)
     .put(route('estudiantes.update', datosEstudiante.value.id), {
    onSuccess: () => {
